...
FIELDNAMES = [
    "label", "workers", "queue_capacity", "backpressure", "devices", "rate",
...
def run_one(gateway_bin, simulator_bin, sink_file, label, *,
            workers=4, queue_capacity=2000, backpressure="block",
    ...
    result = {
        "label": label, "workers": workers, "queue_capacity": queue_capacity,
...
def build_matrix():
    ...
    for policy in ("block", "drop-oldest", "drop-newest"):
        rows.append(dict(label=f"backpressure={policy}", backpressure=policy,
                          queue_capacity=50, devices=500, rate=50.0))
...
def write_markdown(path, rows):
    ...
    for row in rows:
        lines.append(
            f"| {row.get('label','')} | {row.get('workers','')} | "
...
def main():
    ...
    gateway_bin = build_dir / "gateway" / "edgeflow-gateway"
    simulator_bin = build_dir / "simulator" / "edgeflow-simulator"
    if not gateway_bin.exists() or not simulator_bin.exists():
        print(f"error: build gateway/simulator binaries first "
              f"(expected at {gateway_bin} and {simulator_bin})", file=sys.stderr)
        return 1
    ...
    for row_config in build_matrix():
        ...
        result = run_one(gateway_bin, simulator_bin, sink_file, **row_config)
        if result["error"]:
